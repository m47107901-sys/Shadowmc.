...
def create_embed(title, description="", color=0x1a1a1a):
    embed = discord.Embed(
        title=truncate_text(f"â­ UnixNodes - {title}", 256),
...
class OSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        try:
            ...
            add_field(dm_embed, "Management", "â€¢ Use `!manage` to start/stop/reinstall your UnixNodes VPS\nâ€¢ Use `!manage` â†’ SSH for terminal access\nâ€¢ Contact UnixNodes admin for upgrades or issues", False)
...
class ManageView(discord.ui.View):
    ...
    async def create_vps_embed(self, index):
        ...
        add_field(embed, "ðŸŽ® Controls", "Use the buttons below to manage your UnixNodes VPS", False)
        return embed
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'stats':
            ...
            disk_usage = await get_container_disk(container_name)
            uptime = await get_container_uptime(container_name)
            stats_embed = create_info_embed("ðŸ“ˆ UnixNodes Live Statistics", f"Real-time stats for `{container_name}`")
        ...
        if action == 'reinstall':
            ...
            confirm_embed = create_warning_embed("UnixNodes Reinstall Warning",
                f"âš ï¸ **WARNING:** This will erase all data on VPS `{container_name}` and reinstall {os_version}.\n\n"
...
async def user_info(ctx, user: discord.Member):
    ...
    if vps_list:
        ...
        add_field(embed, "ðŸ–¥ï¸ UnixNodes VPS Information", vps_summary, False)
        ...
        add_field(embed, "ðŸ–¥ï¸ UnixNodes VPS Information", "**No VPS owned**", False)
    is_admin_user = user_id == str(MAIN_ADMIN_ID) or user_id in admin_data.get("admins", [])
    add_field(embed, "ðŸ›¡ï¸ UnixNodes Admin Status", f"**{'Yes' if is_admin_user else 'No'}**", False)
    await ctx.send(embed=embed)
...
async def vps_info(ctx, container_name: str = None):
    if not container_name:
        ...
        embed = create_embed(f"ðŸ–¥ï¸ UnixNodes VPS Information - {container_name}", f"Details for VPS owned by {found_user.mention}{suspended_text}{whitelisted_text}", 0x1a1a1a)
...
async def execute_command(ctx, container_name: str, *, command: str):
    ...
    try:
        ...
        if error.strip():
            if len(error) > 1000:
                error = error[:1000] + "\n... (truncated)"
            add_field(embed, "âš ï¸ Error", f"```\n{error}\n```", False)
        add_field(embed, "ðŸ”„ Exit Code", f"**{proc.returncode}**", False)
        await ctx.send(embed=embed)
    except Exception as e:
...
async def vps_stats(ctx, container_name: str):
    ...
    try:
        ...
        add_field(embed, "ðŸ’¾ Disk Usage", f"**{disk_usage}**", True)
        add_field(embed, "â±ï¸ Uptime", f"**{uptime}**", True)
        add_field(embed, "ðŸŒ Network Usage", f"**{network_usage}**", False)
        ...
        if found_vps:
            suspended_text = " (SUSPENDED)" if found_vps.get('suspended', False) else ""
            whitelisted_text = " (WHITELISTED)" if found_vps.get('whitelisted', False) else ""
            add_field(embed, "ðŸ“‹ Allocated Resources",
...
async def vps_logs(ctx, container_name: str, lines: int = 50):
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            embed = create_embed(f"ðŸ“‹ UnixNodes Logs - {container_name}", f"Last {lines} log lines", 0x1a1a1a)
